...
export default function TemplatesPopup({ popup, setPopup, templateInfo }: Props) {
	return (
		<>
			...
			<AnimatePresence>
				{popup && (
					<>
						<motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.2 }} className="absolute top-1/2 left-1/2 z-150 w-120 max-md:w-11/12  bg-alt-background-color p-8 rounded-xl border border-border-color  transform -translate-x-1/2 -translate-y-1/2">
							...
							<div className="flex flex-col gap-4">
								...
								<Link href={templateInfo.code ? `/builder/${templateInfo.templateId}` : ""} className={`${!templateInfo.code && "opacity-50 cursor-not-allowed"} w-full h-18 px-0 rounded-xl relative overflow-visible `}>
									<div className="flex items-center gap-3 w-full p-4 bg-border-color  rounded-xl">
										<div className="bg-alt-border-color p-2.5 rounded-lg">
											<Edit />
										</div>
							...
							<div>
								...
								<div className="flex items-center w-full h-10 bg-border-color border border-alt-border-color rounded-lg opacity-80">
									...
									<button className="px-3 cursor-pointer" onClick={() => navigator.clipboard.writeText(templateInfo.link)}>
