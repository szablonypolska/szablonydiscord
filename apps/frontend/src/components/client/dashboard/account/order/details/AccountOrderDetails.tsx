...
import AccountOrderDetailsOrderData from "./AccountOrderDetailsData"
...
export default function AccountOrderDetails({ orders, selectedItem, setSelectedItem, setOrders }: Props) {
	...
	const order = useMemo(() => orders.find(o => o.id === selectedItem) || null, [orders, selectedItem])
	...
	return (
		<>
			{viewPopup && <AccountOrderRefundPopupController orderId={order?.id || ""} eligibleProducts={eligibleProducts} setViewPopup={setViewPopup} viewPopup={viewPopup} setOrders={setOrders} />}
			<AnimatePresence>
				{order && (
					<>
						...
						<motion.div initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.2 }} exit={{ x: 100, opacity: 0 }} className="flex flex-col fixed top-0 right-0 h-screen w-[40rem] max-md:w-full bg-background border-l border-border-color scrollbar scrollbar-thumb-alt-border-color scrollbar-track-border-color  overflow-y-auto z-50">
							<div className="flex items-center justify-between p-5 sticky bg-box-color border-b border-border-color top-0">
								<button className="cursor-pointer text-primary-color p-2 rounded-lg hover:bg-border-color" onClick={() => setSelectedItem(null)}>
