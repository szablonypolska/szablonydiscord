...
export default function Cards({ data, settingsData, className }: { data: Template; settingsData?: UserSettings | null; className?: string }) {
	return (
		<div className={clsx("card flex flex-col bg-background border border-border-color p-5 rounded-lg", className)} key={data.id}>
			...
			{settingsData && settingsData.templatesDetail && (
				<div className="flex gap-4 mt-3">
					<div className="flex items-center gap-1 text-sm bg-alt-background-color border border-border-color px-2 py-1 rounded-md">
						...
						<span>{data.channelCount} kanałów</span>
					</div>
					<div className="flex items-center gap-1 text-sm bg-alt-background-color border border-border-color px-2 py-1 rounded-md">
