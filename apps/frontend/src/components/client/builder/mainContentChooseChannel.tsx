	...
	return (
		<div className="w-1/2  max-md:w-full">
			...
			<div className="relative mt-2 w-full">
				<button className="flex items-center justify-between bg-border-color w-full border border-alt-border-color rounded-lg p-2 text-left text-sm hover:border-primary-color" onClick={() => setViewChannelDecoration(!viewChannelDecoration)}>
				...
				{viewChannelDecoration && (
					<div className="absolute flex flex-col items-start bg-box-color p-2  rounded-lg w-11/12 mt-2 border border-border-color h-80  z-30 max-md:w-full" ref={elementRef}>
						<div className="w-full">
							<button onClick={() => setCurrentChannelDecoration("┆・📜・regulamin")} className={`${"┆・📜・regulamin" === currentChannelDecoration && "bg-primary-color w-full rounded-lg text-left px-2"} w-full text-left px-4`}>
								<pre className="my-2 text-sm">┆・📜・regulamin (domyślny)</pre>
								<p></p>
							</button>
							<button onClick={() => setCurrentChannelDecoration("inny")} className={`${"inny" === currentChannelDecoration && "bg-primary-color w-full rounded-lg text-left px-2"} w-full text-left px-4 mt-2`}>
