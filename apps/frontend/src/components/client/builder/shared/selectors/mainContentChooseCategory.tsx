	...
	return (
		<div className="w-1/2 max-md:w-full">
			...
			<div className="relative mt-2 w-full">
				<button className="flex items-center justify-between bg-border-color w-full border border-alt-border-color rounded-lg p-2 text-left text-sm hover:border-primary-color cursor-pointer" onClick={() => setViewCategoryDecoration(true)}>
					...
					<AnimatePresence>
						<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="absolute flex flex-col items-start bg-box-color p-2  rounded-lg w-11/12 mt-2 border border-border-color h-80  z-30 max-md:w-full" ref={elementRef}>
							<div className="w-full">
								<button onClick={() => setCurrentCategoryDecoration("━━≫INFORMACJE≪━━")} className={`${"━━≫INFORMACJE≪━━" === currentCategoryDecoration && "bg-primary-color w-full rounded-lg text-left px-2"} w-full text-left px-4 cursor-pointer`}>
									<pre className="my-2 text-sm">━━≫INFORMACJE≪━━ (domyślny)</pre>
									<p></p>
								</button>
								<button onClick={() => setCurrentCategoryDecoration("inny")} className={`${"inny" === currentCategoryDecoration && "bg-primary-color w-full rounded-lg text-left px-2"} w-full text-left px-4 mt-2 cursor-pointer `}>
							...
							<div className="scrollbar scrollbar-thumb-alt-border-color scrollbar-track-border-color overflow-y-scroll w-full">
								{decorationCategory.map((el, index) => (
									<button onClick={() => setCurrentCategoryDecoration(el.style)} key={index} className={`${el.style === currentCategoryDecoration && "bg-primary-color w-full rounded-lg text-left px-2"} w-full truncate text-left px-4 my-0.5 cursor-pointer`}>
