...
export default function PreviewTemplate() {
	...
	const channels: Channel[] = findStage(builderData, BuilderStageType.CHANNEL)?.channel?.channel ?? []
	const categories: Category[] = findStage(builderData, BuilderStageType.CATEGORY)?.category?.category ?? []
	const roles: Role[] = findStage(builderData, BuilderStageType.ROLES)?.role?.role ?? []

	const allChannels = [...channels, ...categories]

	return (
		<div className="flex flex-col items-center w-full ">
			<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-alt-background-color w-full p-8 rounded-xl border border-border-color max-lg:p-5">
				<header>
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-4 w-full">
							{builderData.title ? <h1 className="text-2xl max-w-120 truncate max-lg:max-w-96">{builderData.title}</h1> : <div className="w-72 h-8 bg-border-color rounded-lg animate-pulse"></div>}

							<div className="flex gap-1 max-sm:hidden">
								<span className="px-4 py-1 bg-border-color w-fit text-sm rounded-full">AI</span>
							</div>
						</div>
					...
					{builderData.description ? <p className="text-text-color text-lg md:w-9/12 max-md:w-full">{builderData.description}</p> : <div className="w-md h-6 bg-border-color rounded-lg animate-pulse"></div>}
				</header>
					...
					<div className="flex gap-2 max-sm:mt-2 max-sm:w-full">
						<Tooltip content="Kanały tekstowe" className="p-1 px-3 bg-border-color rounded-xl" delay={300}>
							<div className="flex items-center gap-1 px-3 h-12 bg-border-color w-fit rounded-xl max-sm:grow max-sm:justify-center">
								...
								<p className="text-xl">{channels.length}</p>
							</div>
						</Tooltip>
						...
						<Tooltip content="role" className="p-1 px-3 bg-border-color rounded-xl" delay={300}>
							<div className="flex items-center gap-1 px-3 h-12 bg-border-color w-fit rounded-xl max-sm:grow max-sm:justify-center">
								<Users />
								<p className="text-xl">{roles.length}</p>
							</div>
						</Tooltip>
					</div>
				</section>
				...
				<section className="flex items-center gap-3 mt-8 max-sm:flex-col max-sm:w-full max-sm:mt-5">
					<Button className="flex items-center bg-primary-color px-8 py-6 rounded-xl max-sm:w-full opacity-30 cursor-not-allowed" disabled>
						<Plus />
						Użyj szablonu
					</Button>

					<Button className="flex items-center bg-advice-bot px-8 py-6 rounded-xl max-sm:w-full opacity-30 cursor-not-allowed" disabled>
						<Bot />
						Przejdź do podglądu szablonu
					</Button>
				</section>
			</motion.div>
			<PreviewVisualization allChannels={allChannels} roles={roles} categories={categories} channels={channels} />
		</div>
	)
}
