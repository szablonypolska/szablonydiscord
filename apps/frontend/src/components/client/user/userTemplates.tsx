...
export default function TemplatesProfile({ data }: Props) {
	...
	const renderItem = ({ ref, key, value, isActive, onNext, onPrevious, setPage, className }: PaginationItemRenderProps) => {
		if (value === PaginationItemType.NEXT) {
			return (
				<button key={key} className={cn(className, "w-9 h-9 bg-box-color-dashboard border border-border-color")} onClick={onNext}>
					<ArrowRight size="15" />
				</button>
			)
		}
		...
		if (value === PaginationItemType.PREV) {
			return (
				<button key={key} className={cn(className, " w-9 h-9 bg-box-color-dashboard border border-border-color")} onClick={onPrevious}>
					<ArrowLeft size="15" />
				</button>
			)
		}
		...
		return (
			<button key={key} ref={ref} className={cn(className, "bg-box-color-dashboard border border-border-color", isActive && "text-white bg-primary-color ")} onClick={() => setPage(value)}>
				{value}
			</button>
		)
	}
	return (
		<>
			<div className="w-full bg-box-color border border-border-color pb-7 rounded-xl">
